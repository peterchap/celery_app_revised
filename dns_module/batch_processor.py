...
def _dnsrecord_to_expanded_row(rec: DNSRecord) -> Dict[str, Any]:
    ...
    return {
        ...
        # Hygiene fields fall back to the records dict: the batch fetch_domain
        # (Phase 3.8) stores what it collects in rec.records, not as attrs.
        "has_mta_sts": bool(getattr(rec, "has_mta_sts", False) or rd.get("has_mta_sts") or False),
        "mta_sts_txt": getattr(rec, "mta_sts_txt", "") or rd.get("mta_sts_txt") or "",
        ...
        "smtp_cert_issuer": getattr(rec, "smtp_cert_issuer", "") or rd.get("smtp_cert_issuer") or "",
        "https_cert_ok": getattr(rec, "https_cert_ok", None) if getattr(rec, "https_cert_ok", None) is not None else rd.get("https_cert_ok"),
        "https_cert_days_left": getattr(rec, "https_cert_days_left", None) if getattr(rec, "https_cert_days_left", None) is not None else rd.get("https_cert_days_left"),
        "https_cert_issuer": getattr(rec, "https_cert_issuer", "") or rd.get("https_cert_issuer") or "",
